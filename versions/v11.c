 ...
 * PostgreSQL extension - FULLY DEBUGGED VERSION
 * 
 ...
 * 
 * FIXED: Underscore pattern matching with proper length validation
 */
 ...
 /* FIXED: Helper to count pattern length including underscores */
 ...
 /* FIXED: match_at_pos with proper length validation */
 static RoaringBitmap* match_at_pos(const char *pattern, int start_pos)
 {
     ...
     int plen = strlen(pattern);
     int i;
     int required_length = start_pos + plen; /* Minimum string length needed */
     ...
     for (i = 0; i < plen; i++)
     {
         if (pattern[i] == '_')
         {
             /* Underscore means "any character must exist at this position"
              * This is enforced by the length check above */
             pos++;
             continue;
         }
 ...
 /* FIXED: match_at_neg_pos with proper length validation */
 static RoaringBitmap* match_at_neg_pos(const char *pattern, int end_offset)
 {
     ...
     int plen = strlen(pattern);
     int i, pos;
     int required_length = plen; /* Minimum string length needed */
