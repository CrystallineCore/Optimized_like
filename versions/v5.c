...
static uint32_t* optimized_query(const char *pattern, uint64_t *result_count)
{
    ...
    if (info->slice_count == 1)
    {
        ...
        slice_len = count_non_wildcard(slice);
    ...
    else
    {
        ...
        for (i = 0; i < info->slice_count; i++)
        {
            ...
            {
                RoaringBitmap *temp2 = roaring_and(candidates, temp);
                roaring_free(candidates);
                roaring_free(temp);
                candidates = temp2;
            }
        ...
        /* For patterns with middle slices, we can only guarantee candidates
         * For fully accurate results without verification, we keep current result
         * The bitmaps guarantee necessary conditions but not sufficient for complex middle patterns */
    }
