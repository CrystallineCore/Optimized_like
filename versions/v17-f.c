 ...
 * FULLY OPTIMIZED VERSION - Position-range-constrained dynamic programming
 * FIXED: Syntactical errors and undercounting bugs corrected
 */
 ...
 /* Count pattern length including underscores but excluding % wildcards */
 ...
 /* Match pattern at exact position with length validation */
 ...
 /* Match pattern at exact position from end with length validation */
 ...
 /* OPTIMIZED: Position-range-constrained dynamic programming for multi-slice patterns */
 static RoaringBitmap* match_multislice_with_position_ranges(PatternInfo *info)
 {
     ...
     /* Calculate slice lengths and total */
     ...
     /* Allocate DP table: dp_table[slice_idx][position] = bitmap of valid docs */
     ...
     /* PHASE 1: Initialize first slice positions */
     {
         const char *first_slice = info->slices[0];
         int first_len = slice_lengths[0];
         int start_pos = 0;
         
         /* Iterate over possible string lengths - FIXED: include max_len */
         for (len = total_length; len <= max_len && len < global_index->length_idx.max_length; len++)
         {
             ...
             /* First slice can appear at positions [start_pos, len - total_length] */
             int max_first_pos = len - total_length;
             
             /* FIXED: For patterns NOT starting with %, first slice MUST be at position 0 */
             ...
             for (pos = start_pos; pos <= max_first_pos && pos <= max_len - first_len; pos++)
             {
                 ...
                 {
                     RoaringBitmap *filtered = roaring_and(matches, length_filter);
                     ...
                     if (!roaring_is_empty(filtered))
                     {
                         if (!dp_table[0][pos])
                         {
                             dp_table[0][pos] = filtered;
                         }
                         else
                         {
                             RoaringBitmap *merged = roaring_or(dp_table[0][pos], filtered);
                             roaring_free(dp_table[0][pos]);
                             roaring_free(filtered);
                             dp_table[0][pos] = merged;
                         }
                     }
                     else
                     {
     ...
     /* PHASE 2: Dynamic programming for subsequent slices */
     for (i = 1; i < slice_count; i++)
     {
         ...
         /* Calculate remaining length after current slice */
         int remaining_length = 0;
         for (k = i + 1; k < slice_count; k++)
             remaining_length += slice_lengths[k];
         
         /* For each position where previous slice was found */
         for (j = 0; j <= max_len; j++)
         {
             if (!dp_table[i - 1][j] || roaring_is_empty(dp_table[i - 1][j]))
                 continue;
             
             /* Current slice must start after previous slice ends */
             ...
             for (len = total_length; len <= max_len && len < global_index->length_idx.max_length; len++)
             {
                 ...
                 /* Current slice can appear at positions [min_curr_pos, len - remaining_length - current_len] */
                 int max_curr_pos = len - remaining_length - current_len;
                 
                 if (max_curr_pos < min_curr_pos)
                     continue;
                 
                 /* Try all valid positions for current slice */
                 for (pos = min_curr_pos; pos <= max_curr_pos && pos <= max_len - current_len; pos++)
                 {
                     ...
                     if (!roaring_is_empty(matches))
                     {
                         /* Intersect with: docs that had previous slice at position j */
                         RoaringBitmap *temp1 = roaring_and(matches, dp_table[i - 1][j]);
                         roaring_free(matches);
                         
                         if (!roaring_is_empty(temp1))
                         {
                             /* Also intersect with length constraint */
                             RoaringBitmap *filtered = roaring_and(temp1, length_filter);
                             roaring_free(temp1);
                             ...
                             if (!roaring_is_empty(filtered))
                             {
                                 if (!dp_table[i][pos])
                                 {
                                     dp_table[i][pos] = filtered;
                                 }
                                 else
                                 {
                                     RoaringBitmap *merged = roaring_or(dp_table[i][pos], filtered);
                                     roaring_free(dp_table[i][pos]);
                                     roaring_free(filtered);
                                     dp_table[i][pos] = merged;
                                 }
                             }
                             else
                             {
     ...
     /* PHASE 3: Collect results from last slice */
     result = roaring_create();
     {
         ...
         {
             /* Pattern ends with % - can end anywhere */
             for (j = 0; j <= max_len; j++)
             {
                 if (dp_table[last_idx][j] && !roaring_is_empty(dp_table[last_idx][j]))
                 {
                     RoaringBitmap *temp = roaring_or(result, dp_table[last_idx][j]);
                     roaring_free(result);
                     result = temp;
                 }
             }
         ...
         {
             /* Pattern does NOT end with % - must end exactly at string end */
             /* FIXED: include max_len and add bounds check */
             for (len = total_length; len <= max_len && len < global_index->length_idx.max_length; len++)
             {
                     ...
                     !roaring_is_empty(dp_table[last_idx][required_pos]))
                 {
                     ...
                     if (length_filter && !roaring_is_empty(length_filter))
                     {
                         ...
                         if (!roaring_is_empty(filtered))
                         {
                             RoaringBitmap *temp = roaring_or(result, filtered);
                             roaring_free(filtered);
                             roaring_free(result);
                             result = temp;
                         }
 ...
 static RoaringBitmap* get_length_range(int min_len, int max_len)
 {
     ...
     /* FIXED: Better handling of max_len bounds */
 ...
 static uint32_t* optimized_query(const char *pattern, uint64_t *result_count)
 {
     ...
     if (info->slice_count == 1)
     {
         ...
         /* Case: %pattern% - OPTIMIZED with position sliding */
         else
         {
             result = roaring_create();
             
             /* Try pattern at all possible positions */
             ...
             for (i = 0; i <= max_pos && i <= global_index->max_len; i++)
             {
                 ...
                 if (!roaring_is_empty(matches))
                 {
                     RoaringBitmap *filtered = roaring_and(matches, candidates);
                     roaring_free(matches);
                     
                     if (!roaring_is_empty(filtered))
                     {
                         RoaringBitmap *temp_result = roaring_or(result, filtered);
                         roaring_free(result);
                         roaring_free(filtered);
                         result = temp_result;
                     }
     ...
     /* Multiple slices - USE POSITION-RANGE-CONSTRAINED DYNAMIC PROGRAMMING */
     else
     {
 ...
 Datum build_optimized_index(PG_FUNCTION_ARGS)
 {
     ...
     elog(INFO, "OPTIMIZED: Position-range-constrained dynamic programming enabled");
     elog(INFO, "Algorithm: O(slices * max_len^2) bitmap ops - NO string verification needed");
 ...
 Datum optimized_like_status(PG_FUNCTION_ARGS)
 {
     ...
     appendStringInfo(&buf, "  FIXED BUGS:\n");
     appendStringInfo(&buf, "  ✓ Array bounds: max_len now properly included\n");
     appendStringInfo(&buf, "  ✓ NULL checks: Safe array access with bounds validation\n");
     appendStringInfo(&buf, "  ✓ Position locking: Non-leading %% patterns lock first slice to pos 0\n");
     appendStringInfo(&buf, "  ✓ Loop bounds: Changed < to <= for inclusive max_len\n");
     appendStringInfo(&buf, "\n");
     appendStringInfo(&buf, "  OPTIMIZED ALGORITHMS:\n");
     appendStringInfo(&buf, "  ✓ Single-slice %%pattern%%: Position sliding O(max_len) bitmap ops\n");
     appendStringInfo(&buf, "  ✓ Multi-slice %%bc%%d%%efg%%: Dynamic programming with position ranges\n");
     appendStringInfo(&buf, "  ✓ Complexity: O(slices * max_len^2 * distinct_lengths) bitmap ops\n");
     appendStringInfo(&buf, "  ✓ NO string verification overhead - pure bitmap algebra!\n");
     appendStringInfo(&buf, "\n");
     
     #ifdef HAVE_ROARING
     appendStringInfo(&buf, "  Backend: Native Roaring library (compressed bitmaps)\n");
     #else
