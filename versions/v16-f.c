 ...
 * FULLY FIXED VERSION - Position-based constraint optimization added
 */
 ...
 /* Count pattern length including underscores */
 ...
 /* NEW: Position-constrained verification for multi-slice patterns */
 static RoaringBitmap* verify_multislice_pattern_constrained(RoaringBitmap *candidates, PatternInfo *info)
 {
     uint64_t count, i, j;
     uint32_t *indices;
     uint32_t idx;
     ...
     int *slice_lengths;
     int total_slice_length = 0;
     int str_len, min_pos, max_pos;
     
     /* Calculate slice lengths */
     slice_lengths = (int *)palloc(info->slice_count * sizeof(int));
     for (j = 0; j < info->slice_count; j++)
     {
         slice_lengths[j] = pattern_length_with_underscores(info->slices[j]);
         total_slice_length += slice_lengths[j];
     }
     ...
     for (i = 0; i < count; i++)
     {
         ...
         /* Calculate cumulative length from start */
         int cumulative_from_start = 0;
         
         for (j = 0; j < info->slice_count; j++)
         {
             const char *slice = info->slices[j];
             int slice_len = slice_lengths[j];
             
             /* Calculate remaining slices length after this one */
             int remaining_length = 0;
             int k;
             for (k = j + 1; k < info->slice_count; k++)
                 remaining_length += slice_lengths[k];
             
             /* Position constraints:
              * - Must start at or after: cumulative_from_start
              * - Must end at or before: str_len - remaining_length
              */
             min_pos = search_start - str;
             max_pos = str_len - remaining_length - slice_len;
             
             /* If constraints are impossible, skip this record */
             if (min_pos > max_pos)
             {
                 all_found = false;
                 break;
             ...
             /* Check if match is within valid position range */
             int match_position = match_pos - str;
             if (match_position > max_pos)
             {
                 all_found = false;
                 break;
             }
             
             /* Move search_start to end of current match */
             ...
             cumulative_from_start += slice_len;
         }
 ...
 static uint32_t* optimized_query(const char *pattern, uint64_t *result_count)
 {
     ...
     /* Multiple slices - USE POSITION-CONSTRAINED VERIFICATION */
     else
     {
         /* Calculate min length properly */
         min_len = 0;
         for (i = 0; i < info->slice_count; i++)
         ...
         /* USE POSITION-CONSTRAINED VERIFICATION */
         RoaringBitmap *verified = verify_multislice_pattern_constrained(result, info);
         roaring_free(result);
         result = verified;
     }
 ...
 Datum build_optimized_index(PG_FUNCTION_ARGS)
 {
     ...
     elog(INFO, "OPTIMIZED: Position-based constraint verification enabled");
 ...
 Datum optimized_like_status(PG_FUNCTION_ARGS)
 {
     ...
     initStringInfo(&buf);
     appendStringInfo(&buf, "Roaring Bitmap Index Status (POSITION-CONSTRAINED):\n");
     ...
     appendStringInfo(&buf, "  OPTIMIZED: Position-based constraint verification\n");
     appendStringInfo(&buf, "    - Multi-slice patterns use position ranges\n");
     appendStringInfo(&buf, "    - Example: %%aef%%bcd%% restricts aef to [0, n-3] and bcd to [3, n]\n");
